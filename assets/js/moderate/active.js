...
var entityInformationVal = 1;
	database.ref("offers").on("child_added", function(moderate){
		if (moderate.val().status === "active"){
			...
			$("<div>").addClass("row company" + " " + moderate.val().company).appendTo(".active")
			.append("<div class='col-md-4 entity-information entity-information-" + entityInformationVal + "'>" +
						...
						"<button class='btn btn-danger deactivate' id='deactivate' company='" + moderate.val().company + "''>Deactivate</button>" +
					"</div>")
